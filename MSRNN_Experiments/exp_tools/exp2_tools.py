...
class DataGenerator :
    def __init__(self, amplitude, frequency, phase, t) :
        ...
        self.noise = [torch.from_numpy(np.random.normal(0, 0.1*amp, self.signals.shape)).float() for amp, signal in zip(amplitude, self.signals)]
        self.noisy_signlas = [signal + noise for signal, noise in zip(self.signals, self.noise)]

        self.scenario1 = torch.cat(
